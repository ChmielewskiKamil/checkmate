...
// MutantLLMAnalysisOutcome holds the detailed result of an LLM analysis attempt for a single mutant.
type MutantLLMAnalysisOutcome struct {
	MutantID     string `json:"mutantId"`               // The ID of the mutant
	Status       string `json:"status"`                 // "COMPLETED", "FAILED_CONTEXT_GEN", "FAILED_LLM_CALL", "PENDING_RETRY"
	LLMResponse  string `json:"llmResponse,omitempty"`  // Raw LLM output; ONLY populated on Status == "COMPLETED"
	ErrorMessage string `json:"errorMessage,omitempty"` // Error message if status indicates failure
	Timestamp    string `json:"timestamp"`              // When the analysis was last attempted
	ModelUsed    string `json:"modelUsed,omitempty"`    // Which LLM model was used for the last attempt
}
...
type AnalyzedFile struct {
	...
	// LLMAnalysisOutcomes stores the detailed outcome of each LLM analysis attempt for mutants in this file.
	// This is where status and error messages will live. Keyed by Mutant ID.
	LLMAnalysisOutcomes map[string]MutantLLMAnalysisOutcome `json:"llmAnalysisOutcomes"`
}
